...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            ...
            critics = np.random.permutation(num_critic_networks)
            next_qs = torch.min(next_qs[critics[0]], next_qs[critics[1]])
        else:
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values: torch.Tensor = reward + (~done) * self.discount * next_qs
