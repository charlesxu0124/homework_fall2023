...
class PGAgent(nn.Module):
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            if self.gae_lambda is None:
                ...
                # # Weights for the cumsum operation
                # weights = (self.gamma * self.gae_lambda * (1 - terminals)).cumprod(dim=0)

                # # Flip the tensors since we want to compute the cumsum in reverse order
                # deltas_flipped = torch.flip(deltas, dims=[0])
                # weights_flipped = torch.flip(weights, dims=[0])

                # # Compute the weighted cumsum
                # advantages_flipped = torch.cumsum(deltas_flipped * weights_flipped, dim=0)

                # # Flip the result to obtain the advantages in the original order
                # advantages = torch.flip(advantages_flipped, dims=[0])
        ...
        if self.normalize_advantages:
            if np.std(advantages) != 0:
                ...
                advantages = np.zeros(advantages.shape)
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        total_discounted_return = sum([self.gamma ** t * rewards[t] for t in range(len(rewards))])
        discounted_return = [total_discounted_return for _ in range(len(rewards))]
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for i in range(len(rewards)):
            discounted_return.append(
                sum([self.gamma ** (t - i) * rewards[t] for t in range(i, len(rewards))])
            )
        return discounted_return
