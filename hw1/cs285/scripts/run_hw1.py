...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        training_logs = []
        pbar = tqdm(range(params['num_agent_train_steps_per_iter']))
        for _ in pbar:
          ...
          ob_batch = ptu.from_numpy(replay_buffer.obs[idx].astype(np.float32))
          ac_batch = ptu.from_numpy(replay_buffer.acs[idx].astype(np.float32))

          # use the sampled data to train an agent
          train_log = actor.update(ob_batch, ac_batch)
          pbar.set_description(f"Train loss: {train_log['Training Loss']}")
